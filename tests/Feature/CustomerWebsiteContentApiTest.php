...
class CustomerWebsiteContentApiTest extends TestCase
{
    ...
    public function test_guests_can_read_customer_safe_website_content(): void
    {
        ...
        CmsPage::query()->where('key', 'about')->update([
            'body' => '<p>About our cafe</p><script>alert(1)</script>',
            'is_published' => true,
        ]);
        ...
        $this->getJson(route('api.v1.content.show'))
            ...
            ->assertJsonPath('data.branding.name', 'Config Cafe')
            ...
            ->assertJsonPath('data.pages.about', '<p>About our cafe</p>')
            ->assertJsonPath(
    ...
    public function test_opening_hours_come_from_cafe_schedule_not_legacy_display_text(): void
    {
        WebsiteSetting::query()->where('key', WebsiteSettingKey::BusinessOpeningHours->value)->update([
            'value' => 'Legacy display hours that must not appear',
        ]);
        ...
        $this->assertStringContainsString('09:15', $hours);
        $this->assertStringContainsString('17:45', $hours);
        $this->assertStringNotContainsString('Legacy display hours that must not appear', $hours);
    }
    ...
    public function test_delivery_disclaimer_setting_overrides_config(): void
    {
        ...
        WebsiteSetting::query()->where('key', WebsiteSettingKey::HeroSubtitle->value)->update([
            'value' => 'CAFFEINE TILL COFFIN.',
        ]);

        $this->getJson(route('api.v1.content.show'))
            ->assertOk()
            ->assertJsonPath('data.hero.subtitle', 'CAFFEINE TILL COFFIN.')
            ->assertJsonPath('data.branding.tagline', 'CAFFEINE TILL COFFIN.')
