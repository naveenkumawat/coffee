...
@if ($session->hasFinalizedBill()
    ...
    <form
        method="POST"
        action="{{ route('operator.dining-sessions.cash.receive', $session) }}"
...
@if ($canResumeOrdering)
    <form
        method="POST"
        action="{{ route('operator.dining-sessions.reopen', $session) }}"
...
@if ($session->status !== DiningSessionStatus::Closed)
    <form
        method="POST"
        action="{{ route('operator.dining-sessions.close', $session) }}"
