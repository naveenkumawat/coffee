...
- **Realtime (R1):** self-hosted Laravel Reverb + Echo. Broadcasting default may be `log`/`null` when Reverb is down — REST remains authoritative. Private channels authorized in `routes/channels.php` for `user.{id}` and staff `role.*` (administrator = owner|manager). Shared Blade bootstrap: `resources/views/internal/partials/realtime-bootstrap.blade.php` + Vite `resources/js/realtime.js`. PWA: singleton `customer-pwa/src/realtime/RealtimeConnection.ts` (StrictMode-safe; disconnect on logout). Probe event `RealtimeConnectionProbe` proves auth/delivery with a minimal DTO. Business notification events are deferred to R1.2+.
