...
- **Operational notifications (R1.2 + R1.3A + R1.3B + R1.4 + R1.5 + R1.6):** Domain tables `operational_notifications` + `operational_notification_recipients` are distinct from Laravel’s `notifications` table (legacy staff DB bell retained as mark-legacy-read only). R1.3A wires domain events → `OperationalBusinessNotificationPublisher`. R1.3B adds shared UI: Blade `resources/js/notifications/*` (store, reminder, sound, tab leader, drawer/toasts) loaded for all internal panels; waiter PWA bell/drawer foundation. Reminder policy: 30s interval constant; eligible actionable types only; leader tab alone presents sound/reminder toast and POSTs `/reminded`. ACK: delivered on socket, seen when visible, read on open, acknowledge on explicit button — dismiss toast ≠ resolve. Reconnect/online/visibility sync uses REST as authority (R1.5 coalesces duplicate sync triggers; client dedupes socket event ids). Dining ready-to-serve reminders may continue until Completed/Cancelled/Rejected (no Served state yet — L1 decision deferred to R1.7). **R1.4** adds customer-facing `customer.*` types. **R1.5** adds advisory staff presence (`presence-ops` + TTL heartbeat; unique-user counts), no-staff escalations for missing Barista/Chef/Waiter online, and inventory/refill operational notifications + `.inventory.ops` role signals (Admin/Operator overview; Barista where relevant). Presence is never business authority. **R1.6** adds scoped `dining-session.{id}` / `table.{id}` channels and `DiningRealtimePublisher` `.dining.ops` signals (session open/update/round/prep/all-ready/bill/payment/close/table release). Clients soft-reconcile via REST; multi-Waiter concurrency remains server-authoritative; payloads omit money/recipes/customer secrets.
