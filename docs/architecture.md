...
- **Operational notifications (R1.2):** Domain tables `operational_notifications` + `operational_notification_recipients` are distinct from Laravel’s `notifications` table (staff in-app bell via `StaffNotificationDispatcher`). `OperationalNotificationService` owns create/attach recipients/broadcast-ready payload/ACK lifecycle/resolve/reminders; repository owns persistence/queries. Team audiences expand roles → active users (one recipient row per user). First valid business resolve sets notification `resolved_at` for everyone; per-recipient timestamps stay. Lifecycle: `created_at` → `broadcast_at` (dispatch attempted) → `delivered_at` (client ACK) → `first_seen_at` → `read_at` → `acknowledged_at`; `resolved_at` is condition-level. ACK endpoints are idempotent (first timestamp wins). Delays are computed only. Broadcast: `OperationalNotificationBroadcasted` after commit on `private-user.{id}`; WS failure must not fail persistence. Business order/payment/preparation hooks arrive in R1.3+.
