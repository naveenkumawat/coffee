...
- **Realtime (R1):** self-hosted Laravel Reverb + Echo. Broadcasting default may be `log`/`null` when Reverb is down — REST remains authoritative. Private channels authorized in `routes/channels.php` for `user.{id}` and staff `role.*` (administrator = owner|manager). Shared Blade bootstrap: `resources/views/internal/partials/realtime-bootstrap.blade.php` + Vite `resources/js/realtime.js` + `resources/js/notifications.js`. PWA: singleton `customer-pwa/src/realtime/RealtimeConnection.ts` (StrictMode-safe; disconnect on logout) and `notificationStore` / `api/notifications.ts`. Probe event `RealtimeConnectionProbe` proves auth/delivery with a minimal DTO.
- **Operational notifications (R1.2 + R1.3A):** Domain tables `operational_notifications` + `operational_notification_recipients` are distinct from Laravel’s `notifications` table (staff in-app bell via `StaffNotificationDispatcher`). `OperationalNotificationService` owns create/attach recipients/broadcast-ready payload/ACK lifecycle/resolve/reminders + `createUniqueAndBroadcast` / `resolveOpenForSubject`; repository owns persistence/queries. Unique `idempotency_key` prevents duplicate creates under retries. R1.3A: existing domain events (`OrderPlaced`, `OrderPaymentProofReceived`/`Rejected`, `OrderStatusChanged`, `OrderPreparationStatusChanged`) → thin `WireOperational*` listeners → `OperationalBusinessNotificationPublisher` (never from controllers). Types/resolution: cash place → `order.requires_attention`; payment confirm → `order.requires_acceptance`; proof → `order.payment_proof_review`; ticket pending → `preparation.ticket_pending` (resolves on first station **Accepted**, not Ready); dining all-Ready → `dining.ready_to_serve` once (resolves on Completed/Cancelled/Rejected — **no served state**); cancel/reject informational + resolves obsolete actionables. Empty active audience does not fail workflows. Broadcast remains generic `.operational.notification`. Bell drawer / 30s reminders deferred to R1.3B.
