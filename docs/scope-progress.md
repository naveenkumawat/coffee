...
- [x] R1.3B realtime notification UI + reminder engine implemented (shared Blade bell/drawer/toasts/sound; 30s ActionReminderManager; multi-tab leader election; `/reminded` endpoint; reconnect sync; waiter PWA foundation; Ready-to-Serve later resolved by L1.1 Served)
- [x] R1.4 customer realtime order-status + notification history implemented (customer.* operational types; owner-only recipients; PWA Account notifications + Account footer unread badge; live order/dining REST reconciliation; no repeating customer reminders; strong Ready/reject/cancel alerts; logout clears state; waiter floating bell retained)
...
- [x] Dining table Call Waiter + escalation implemented (`dining_service_requests`; preferred latest waiter-assisted round placer; 60s escalate to all waiters; claim/complete; customer self-order resolves assistance; preferred waiter ≠ table owner)
